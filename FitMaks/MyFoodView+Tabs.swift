...
// MARK: - Tab Content Views & UI Components
extension MyFoodView {

    var myFoodBackground: some View {
    ...
    func tabButton(title: String, emoji: String, index: Int, color: Color) -> some View {
        Button {
    ...
    func libraryCount(title: String, value: Int, color: Color) -> some View {
        VStack(spacing: 1) {
            Text("\(value)")
    ...
    func mealRow(_ recipe: SavedRecipe) -> some View {
        HStack(spacing: 11) {
    ...
    func rowIconButton(systemName: String, color: Color, action: @escaping () -> Void) -> some View {
    ...
    func selectedTabGradient(for color: Color) -> LinearGradient {
        let theme = AppTheme.current
        if theme == .originalV2 {
            return LinearGradient(
    ...
    func foodSearchBar(color: Color) -> some View {
        HStack(spacing: 10) {
    ...
    var fridgeCategoryChips: some View {
    ...
    var mealCategoryChips: some View {
        LazyVGrid(columns: Array(repeating: GridItem(.flexible(), spacing: 6), count: 4), spacing: 6) {
    ...
    func categoryChip(title: String, emoji: String?, isSelected: Bool, color: Color, fillsWidth: Bool = false, action: @escaping () -> Void) -> some View {
        Button(action: action) {
            HStack(spacing: 3) {
    ...
    func libraryEmptyState(systemName: String, title: String, subtitle: String, color: Color) -> some View {
        VStack {
            Spacer()
    ...
    func foodActionButton(title: String, systemName: String, color: Color, isLoading: Bool = false, action: @escaping () -> Void) -> some View {
        Button(action: action) {
            HStack(spacing: 6) {
                ZStack {
                    Circle()
    ...
    func myFoodRowBackground(accent: Color) -> some View {
        let theme = AppTheme.current
    ...
    @ViewBuilder
    func processingBanner(for tab: Int, color: Color) -> some View {
    ...
    func loadingRow(item: ProcessingItem) -> some View {
        ...
        return HStack(spacing: 13) {
            ...
            VStack(alignment: .leading, spacing: 7) {
                Text(item.statusTitle ?? (item.textPrompt != nil ? "Reading text..." : "ShapeForge AI analyzing..."))
                    .font(.subheadline)
                    .fontWeight(.heavy)
                    .foregroundColor(.appText)
                    .lineLimit(1)
    ...
    func processingItems(for tab: Int) -> [ProcessingItem] {
        processingItems
